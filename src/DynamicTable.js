...
export default class DynamicTable extends React.Component {
    ...
    updateTable() {
        ...
        let rows = [];
        let counter = 1;
        this.state.entries.forEach((currentEntry, index) => {
            ...
            if ((this.state.checkboxCommunity && currentEntry.isCommunityApp) || (this.state.checkboxCUAHSI && currentEntry.isCuahsiApp) || (this.state.checkboxMy && currentEntry.isPersonalApp)) {
                ...
                if (add_entry && (counter <= this.state.maxInputs)) {
                    counter += 1;
                    rows.push(
                        <Entry key={index} position={counter} metadata={currentEntry}/>
                    )
                }
            }
        });
        ...
        return (
            <div className="library-app">
                ...
                <div className="infinite-scroll">
                    <button type="button" placeholder="Show More" className="load-more btn btn-default"
                            onClick={this.onLoadMore}
                            disabled={this.state.maxInputs >= this.state?.entries.length}>Load
                        More
                    </button>
                </div>
                <br/>
